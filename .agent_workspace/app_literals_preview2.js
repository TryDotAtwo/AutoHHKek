...
function renderStatusStrip(snapshot) {
  ...
  const cards = [
    ...
    ["Резюме", snapshot.selected_resume_title || snapshot.selected_resume_id || "не выбрано", snapshot.hh_resumes?.length ? `РќР° hh.ru РЅР°Р№РґРµРЅРѕ ${snapshot.hh_resumes.length}.` : "Список резюме еще не подтянут.", snapshot.selected_resume_id ? "good" : "warn"],
    ["Фильтры", snapshot.filter_plan?.search_text || "не собраны", snapshot.filter_plan?.planner_backend ? `РџР»Р°РЅРёСЂРѕРІС‰РёРє: ${snapshot.filter_plan.planner_backend}.` : "Фильтры еще не построены.", snapshot.filter_plan?.search_text ? "good" : "warn"],
    ["Оценка", String(snapshot.counts?.assessed || 0), `${snapshot.counts?.fit || 0} РїРѕРґС…РѕРґРёС‚ В· ${snapshot.counts?.doubt || 0} СЃРѕРјРЅРµРЅРёРµ В· ${snapshot.counts?.no_fit || 0} РЅРµ РїРѕРґС…РѕРґРёС‚`, snapshot.counts?.assessed ? "good" : snapshot.analysis_job?.running ? "neutral" : "warn"],
  ];
          ...
          (account) => `
            <article class="resume-card ${activeKey === account.account_key ? "is-active" : ""}">
              <div>
                ...
                <p class="muted">${escapeHtml(account.resume_count ? `${account.resume_count} СЂРµР·СЋРјРµ` : "резюме не определены")}</p>
              </div>
              <div class="resume-card-actions">
...
function renderFilterPlan(snapshot) {
  ...
  return `
    <div class="detail-meta-grid">
      <div class="meta-row"><span>РџРѕРёСЃРєРѕРІС‹Р№ С‚РµРєСЃС‚</span><strong>${escapeHtml(filterPlan.search_text || "не задан")}</strong></div>
      ...
      <div class="meta-row"><span>РќР° hh.ru</span><strong>${escapeHtml(liveStats.total_available ? `${liveStats.total_available} РЅР°Р№РґРµРЅРѕ` : "еще не считали")}</strong></div>
    ...
    <div class="detail-meta-grid detail-meta-grid--rules">
      <div class="note"><strong>РћР±С‰РёРµ РїСЂР°РІРёР»Р°</strong><p>${escapeHtml(systemRules || "Пока не собраны.")}</p></div>
      <div class="note"><strong>РџСЂР°РІРёР»Р° РїРѕР»СЊР·РѕРІР°С‚РµР»СЏ</strong><p>${escapeHtml(userRules || "Пока не собраны.")}</p></div>
    </div>
  `;
}
      ...
      <section class="panel intake-stage">
        ...
        <div class="intake-stage-grid">
          <div class="note">
            ...
            <p>${escapeHtml((context.inferred_roles || []).length ? `Р РѕР»Рё: ${context.inferred_roles.join(", ")}` : "Роли из резюме еще не уточнены.")}</p>
            <p>${escapeHtml((context.inferred_skills || []).length ? `РќР°РІС‹РєРё: ${context.inferred_skills.join(", ")}` : "Навыки из резюме еще не уточнены.")}</p>
          </div>
          <div class="note">
            <strong>Р§С‚Рѕ РѕР±СЏР·Р°С‚РµР»СЊРЅРѕ РЅСѓР¶РЅРѕ СѓР·РЅР°С‚СЊ</strong>
            <p>${escapeHtml((snapshot.setup_summary?.intake_missing || []).length ? `РџРѕРєР° РЅРµ Р·Р°РєСЂС‹С‚С‹: ${(snapshot.setup_summary.intake_missing || []).join(", ")}.` : "Структурные пробелы закрыты, можно переходить дальше.")}</p>
            <p>${escapeHtml(currentQuestion ? `РЎРµР№С‡Р°СЃ РІРѕРїСЂРѕСЃ ${stepIndex + 1} РёР· ${questions.length}.` : "Диалог еще не начат.")}</p>
          </div>
        </div>
      ...
      <section class="panel panel--wide">
        ...
        <div class="focus-grid">
          ...
          <div class="note">
            ...
            <p>${escapeHtml(selectedCard ? `${selectedCard.title} В· ${selectedCard.category_label}` : "Вакансия еще не выбрана.")}</p>
          </div>
        </div>
      </section>
    </div>
    ...
    <div class="intake-overlay__panel">
      <div class="intake-overlay__head">
        ...
        <div class="intake-overlay__progress">
          ...
          <span class="muted">${escapeHtml(confirmationMode ? "После подтверждения откроются поиск и анализ" : question ? `Р’РѕРїСЂРѕСЃ ${Number(dialog.step_index || 0) + 1} РёР· ${(dialog.questions || []).length}` : "Нажмите «Начать опрос»")}</span>
        </div>
      </div>
      <div class="intake-overlay__meta">
        <article class="note">
          ...
          <p>${escapeHtml(context.resume_title || snapshot.selected_resume_title || "Резюме пока не выбрано.")}</p>
          <p>${escapeHtml((context.inferred_roles || []).length ? `Р РѕР»Рё: ${context.inferred_roles.join(", ")}` : "Роли из резюме пока не выделены.")}</p>
          <p>${escapeHtml((context.detected_skills || []).length ? `РќР°РІС‹РєРё: ${context.detected_skills.slice(0, 8).join(", ")}` : "Навыки из резюме пока не выделены.")}</p>
        </article>
        <article class="note">
      ...
      <div class="intake-overlay__body">
        <div id="intake-overlay-log" class="intake-overlay__log">
          ${
            confirmationMode
              ? `
                <article class="note"><strong>Р¦РµР»РµРІС‹Рµ СЂРѕР»Рё</strong><p>${escapeHtml((contract.search_targets?.primary_roles || []).join(", ") || "Не заполнено")}</p></article>
                ...
                <article class="note"><strong>Must-have СЃС‚РµРє</strong><p>${escapeHtml((contract.search_targets?.must_have_keywords || []).join(", ") || "Не заполнено")}</p></article>
                <article class="note"><strong>РЎРѕРїСЂРѕРІРѕРґРёС‚РµР»СЊРЅРѕРµ</strong><p>${escapeHtml(`РЇР·С‹Рє: ${contract.cover_letter_policy?.language || "ru"}, С‚РѕРЅ: ${contract.cover_letter_policy?.tone || "деловой"}`)}</p></article>
              `
              : renderList(
                  transcript,
                  (item) => `<article class="chat-message chat-message--${escapeHtml(item.role)}"><span>${escapeHtml(item.role === "assistant" ? "агент" : "вы")}</span><p>${escapeHtml(item.text)}</p></article>`,
  ...
  root.innerHTML = `
    <div class="vacancy-detail-grid">
      <section class="panel panel--detail-main">
        ...
        <div class="note note--decision"><strong>РџРѕС‡РµРјСѓ РїСЂРёРЅСЏС‚Рѕ С‚Р°РєРѕРµ СЂРµС€РµРЅРёРµ</strong><p>${escapeHtml(card.explanation || "Пояснение не сохранено.")}</p></div>
        <div class="detail-block">
      ...
      <aside class="panel panel--detail-side">
        ...
        <div class="note"><strong>РўРµРєСѓС‰РµРµ СЂРµС€РµРЅРёРµ</strong><p>${escapeHtml(card.category_label || "ещё не выбрано")}</p><p class="muted">${escapeHtml(feedback.decided_at ? formatDate(feedback.decided_at) : "автоматическая оценка")}</p></div>
        <div class="field">
        ...
        <div class="note"><strong>РЎС‚Р°С‚СѓСЃ РѕС‚РєР»РёРєР°</strong><p>${escapeHtml(feedback.last_apply_message || "Отклик еще не запускался.")}</p><p class="muted">${escapeHtml(feedback.last_apply_at ? formatDate(feedback.last_apply_at) : "нет запуска")}</p></div>
