...
const categoryMeta = {
  fit: { label: "", hint: "   ", className: "lane--fit" },
  doubt: { label: "", hint: "  ", className: "lane--doubt" },
  no_fit: { label: " ", hint: "   ", className: "lane--no-fit" },
};
...
const reasonMeta = {
  positive: { label: "", className: "reason-lane--positive" },
  neutral: { label: " ", className: "reason-lane--neutral" },
  negative: { label: "", className: "reason-lane--negative" },
};
...
function tryDecodeMojibake(value) {
  if (!value || !/[РСЁÑÐ]/.test(value)) return value;
  try {
...
function looksBetter(original, candidate) {
  ...
  const brokenBefore = (original.match(/[РСЁÑÐ]/g) || []).length + (original.match(/\?{3,}/g) || []).length * 4;
  const brokenAfter = (candidate.match(/[РСЁÑÐ]/g) || []).length + (candidate.match(/\?{3,}/g) || []).length * 4;
...
function currentActivityMessage(snapshot) {
  ...
  if (snapshot.apply_batch_job?.running) {
    return snapshot.apply_batch_job.message || "дет пакетная отправка откликов.";
  }
  if (snapshot.analysis_job?.running) {
    return snapshot.analysis_job.message || "дет анализ и разбор вакансий.";
  }
...
function renderVacancies(snapshot) {
  ...
  if (!isIntakeReady(snapshot)) {
    root.innerHTML = `<section class="panel"><div class="note"><strong>  intake</strong><p>${escapeHtml(" intake  ,     .     onboarding   .")}</p></div></section>`;
    return;
  }
...
function renderActivity(snapshot) {
  ...
  root.innerHTML = `
    <div class="activity-grid">
      ...
      <section class="panel panel--wide">
        <div class="panel-head"><div><span class="panel-kicker">Запуски</span><h2>стория прогонов</h2></div></div>
        <div class="stack">
